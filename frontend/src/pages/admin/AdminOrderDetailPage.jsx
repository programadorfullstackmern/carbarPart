...
import { 
  Container, Row, Col, Card, Button, Spinner, Alert, Badge, Form, Table 
...
const AdminOrderDetailPage = () => {
  ...
  const { updateOrderStatus: updateOrderStatusContext } = useAdmin();
  ...
  const handleStatusUpdate = async () => {
    try {
      setUpdating(true);
      await updateOrderStatusContext(id, status);
      setOrder(prev => ({ ...prev, estado: status }));
    } catch (error) {
        ...
        <div className="d-flex gap-2">
          ...
          >
            {updating ? (
              ...
              <>
                <FaSave className="me-2" /> Actualizar Estado
              </>
            )}
          </Button>
        </div>
